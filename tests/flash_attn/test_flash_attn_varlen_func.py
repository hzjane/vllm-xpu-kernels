...
@pytest.mark.parametrize("head_size,kv_heads,window", [(256, 4, (127, 0)),
                                                       (256, 4, (1023, 1023)),
                                                       (512, 1, (-1, -1))])
@pytest.mark.parametrize("page_size", [64, 128])
@pytest.mark.parametrize("query_lens", [(1, 5), (256, 129)])
@pytest.mark.parametrize("dtype", [torch.float16, torch.bfloat16])
@pytest.mark.parametrize("flags", [(True, False), (False, True)])
@pytest.mark.parametrize("kv_extra", [137, 1792, 7936])
@pytest.mark.parametrize("sm_scale", [None, 1.0])
@torch.inference_mode()
def test_per_sequence_causal_matches_math(head_size, kv_heads, window,
                                         page_size, query_lens, dtype, flags,
                                         kv_extra, sm_scale):
    """Mixed encoder/denoise rows keep their own mask and local window."""
    torch.manual_seed(20260921)
    device = "xpu"
    heads = 8
    kv_lens = [m + kv_extra for m in query_lens]
    pages_per_seq = (max(kv_lens) + page_size - 1) // page_size
    num_pages = 2 * pages_per_seq
    # Match service K/V token interleaving and its non-dense token/page strides.
    cache = torch.randn(num_pages, page_size, kv_heads, 2, head_size,
                        dtype=dtype, device=device)
    key, value = cache[:, :, :, 0], cache[:, :, :, 1]
    table = torch.randperm(num_pages, device=device).to(torch.int32).view(2, -1)
    query = torch.randn(sum(query_lens), heads, head_size,
                        dtype=dtype, device=device)
    cu = torch.tensor([0, query_lens[0], sum(query_lens)], dtype=torch.int32,
                      device=device)
    lengths = torch.tensor(kv_lens, dtype=torch.int32, device=device)
    mask = torch.tensor(flags, dtype=torch.int32, device=device)
    actual = flash_attn_varlen_func(
        query, key, value, max(query_lens), cu, max(kv_lens),
        seqused_k=lengths, block_table=table, causal=True,
        window_size=window, per_seq_causal=mask, softmax_scale=sm_scale)
    expected = []
    offset = 0
    for i, (m, n, causal) in enumerate(zip(query_lens, kv_lens, flags)):
        k = key[table[i].long()].reshape(-1, kv_heads, head_size)[:n]
        v = value[table[i].long()].reshape(-1, kv_heads, head_size)[:n]
        k = k.repeat_interleave(heads // kv_heads, dim=1).float()
        v = v.repeat_interleave(heads // kv_heads, dim=1).float()
        q = query[offset:offset + m].float()
        scale = head_size**-0.5 if sm_scale is None else sm_scale
        scores = torch.einsum("qhd,khd->hqk", q, k) * scale
        qpos = torch.arange(m, device=device)[:, None] + n - m
        kpos = torch.arange(n, device=device)[None, :]
        allowed = torch.ones(m, n, dtype=torch.bool, device=device)
        if causal:
            allowed &= kpos <= qpos
        if window[0] >= 0:
            right = window[1] if causal else window[0]
            allowed &= (kpos >= qpos - window[0]) & (kpos <= qpos + right)
        prob = torch.softmax(scores.masked_fill(~allowed, float('-inf')), -1)
        expected.append(torch.einsum("hqk,khd->qhd", prob, v).to(dtype))
        offset += m
    torch.testing.assert_close(actual, torch.cat(expected), atol=1.5e-2,
                               rtol=1.5e-2)


@torch.inference_mode()
def test_per_sequence_causal_rejects_invalid_mask_before_submit():
    q = torch.randn(2, 8, 256, dtype=torch.float16, device="xpu")
    kv = torch.randn(4, 64, 4, 256, dtype=q.dtype, device=q.device)
    cu = torch.tensor([0, 1, 2], dtype=torch.int32, device=q.device)
    lengths = torch.tensor([64, 64], dtype=torch.int32, device=q.device)
    table = torch.tensor([[0], [1]], dtype=torch.int32, device=q.device)
    for mask in (torch.ones(2), torch.ones(1, device=q.device),
                 torch.ones(2, dtype=torch.float32, device=q.device)):
        with pytest.raises(ValueError, match="per_seq_causal"):
            flash_attn_varlen_func(q, kv, kv, 1, cu, 64, seqused_k=lengths,
                                  block_table=table, per_seq_causal=mask)
